...
export default function LibraryDashboard() {
  ...
  const [finePending, setFinePending] = useState(false);
  ...
  const handleApprove = async () => {
    if (!booksReturned || !cardSubmitted) {
      alert("⚠️ Please verify all library requirements!");
      return;
    }
    if (!remark.trim()) {
...
${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
  ...
  >
    ✕
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Library Department</h1>
        ...
        <div className="p-4 border-t border-blue-700">
          <div className="flex items-center mb-4 px-2">
            ...
            <div className="ml-3 flex-1">
              <p className="text-sm font-medium">Library Admin</p>
  ...
  <p className="text-sm text-gray-600 mt-1">
    {activeTab === "pending" && "Review and process pending applications"}
  ...
  <Clock className="w-5 h-5 text-white" />
                      ...
                      <div>
                        <p className="font-medium text-gray-800">Review Pending Applications</p>
  ...
  <div className="text-sm text-gray-900">
    {app.semester || "N/A"}
                           ...
                           <td className="px-6 py-4">
  {getStatusBadge(
    // app.departments.find((d) => d.name === "library")?.status
    app.departments.find(
  (d) => d.name?.toLowerCase().trim() === "library"
)?.status
  )}
</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-blue-100 mt-1">Library Clearance Verification</p>
                </div>
                <button
                  onClick={resetModal}
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
              ...
              <div className="mb-6">
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Library Verification</h4>
                <div className="space-y-3 bg-blue-50 border border-blue-100 rounded p-4">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={booksReturned}
                      onChange={(e) => setBooksReturned(e.target.checked)}
                      className="w-5 h-5 text-blue-600 rounded border-gray-300"
                    />
                    <span className="text-sm font-medium text-gray-800">All books returned</span>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={!finePending}
                      onChange={(e) => setFinePending(!e.target.checked)}
                      className="w-5 h-5 text-blue-600 rounded border-gray-300"
                    />
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="w-5 h-5 text-blue-600 rounded border-gray-300"
                    />
                    <span className="text-sm font-medium text-gray-800">Library card submitted</span>
                  </label>
                </div>
              </div>
